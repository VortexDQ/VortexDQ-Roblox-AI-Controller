...
// Always include cached game context — only do live round-trip when explicitly needed
async function resolveGameContext(prompt) {
  const cached   = wsManager.getLastKnownState();
  const needsLive= /\b(current|existing|fix|update|modify|edit|change|debug|broken|not working|what's in|show me|what do i have|scan)\b/i.test(prompt);

  const cachedCtx = cached && Object.keys(cached).length ? { lastKnownState: cached, currentGame: wsManager.getCurrentGame() } : null;

  if (!needsLive) return cachedCtx;

  const conn = commandEngine.getActiveConnection();
  if (!conn) return cachedCtx;

  try {
    const gameInfo = await commandEngine.executeCommand({ action: 'GetGameInfo', data: {} });
    return { gameInfo, lastKnownState: cached, currentGame: wsManager.getCurrentGame() };
  } catch (_) {
    return cachedCtx;
  }
}
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    currentGame:   wsManager.getCurrentGame(),
    timestamp:     new Date().toISOString(),
  });
});

// ── Generate AI plan (no Studio execution) ───────────────────────────────────
app.post('/api/generate', async (req, res) => {
  try {
    ...
    if (generation.commands.length > 0) Protocol.validateCommandBatch(generation.commands);
...
app.post('/api/execute', async (req, res) => {
  try {
    ...
    if (generation.commands.length > 0) {
      if (!wsManager.isPluginConnected()) {
        return res.json({
          success:      false,
          pluginOffline: true,
          message:      generation.message || '',
          ...
          results:      [],
          model:        generation.model,
          analysis:     { ...generation.analysis, executionTime: (Date.now() - startTime) + 'ms' },
        });
      }
    ...
    res.json({
      ...
      commands:     generation.commands,
      results:      execution.results,
      analysis:     { ...generation.analysis, executionTime: duration + 'ms' },
    });
  } catch (error) {
...
app.post('/api/analyze', async (req, res) => {
  try {
    ...
    if (!wsManager.isPluginConnected()) {
      return res.json({
        success: false,
        error: 'Plugin not connected — open Roblox Studio with the VortexDQ plugin running.',
      });
    }
    ...
    res.json({ success: true, analysis, examples: analysis.examples, suggestions: analysis.suggestions });
  } catch (error) {
...
app.get('/api/scan/recommendations', (req, res) => {
  ...
  if (!results || !results.findings) {
    return res.json({ message: 'No scan performed yet', recommendations: [] });
  }
...
app.get('/api/analyze/examples', async (req, res) => {
  try {
    ...
    res.json({ examples: analysis.examples || ['create a red part', 'build a platform', 'add a checkpoint'] });
  } catch (_) {
    res.json({ examples: ['create a red part', 'build a platform', 'add a script'] });
  }
});

// ── Models ───────────────────────────────────────────────────────────────────
app.get('/api/models', (req, res) => {
  res.json({ available: modelRouter.getAvailableModels(), active: modelRouter.activeModel });
});
...
app.get('/api/errors/export', (req, res) => {
  ...
  if (format === 'csv') res.type('text/csv').send(data);
...
app.get('/api/game-state', (req, res) => {
  res.json({
    pluginConnected: wsManager.isPluginConnected(),
    currentGame:     wsManager.getCurrentGame(),
...
app.get('/api/status', (req, res) => {
  res.json({
    ...
    connections:  wsManager.getAllConnections().map(conn => ({
      ...
      gameName:   conn.gameName,
    })),
  });
});

// ── Migrations ────────────────────────────────────────────────────────────────
app.get('/api/migrations', (req, res) => res.json(autoMigrator.getStats()));

// ── Version ───────────────────────────────────────────────────────────────────
app.get('/api/version', (req, res) => res.json(autoUpdater.getVersionInfo()));

app.get('/api/version/check', async (req, res) => {
  try {
    res.json(await autoUpdater.checkAndNotify());
  } catch (error) {
...
app.get('/api/version/changelog', (req, res) => {
  res.type('text/markdown').send(autoUpdater.versionManager.getChangelog());
});
...
(async () => { await autoUpdater.checkAndNotify(); })();
...
server.listen(PORT, '127.0.0.1', () => {
  ...
  console.log(`🤖 Models: ${modelRouter.getAvailableModels().filter(m => m.enabled).map(m => m.name).join(', ')}\n`);
});

process.on('unhandledRejection', (reason) => errorTracker.logError(reason, { type: 'unhandledRejection' }));
process.on('uncaughtException',  (error)  => errorTracker.logError(error,  { type: 'uncaughtException'  }));

module.exports = { app, server, wsManager, modelRouter, commandEngine, errorTracker, smartExecutor, gameAnalyzer, gameAntivirus, autoMigrator };
